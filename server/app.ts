...
const getAiBuilderStatus = async (db: VotingDatabase) => {
...
export const createApiFetchHandler = (db: VotingDatabase) => {
  return async (request: Request) => {
    ...
    try {
      ...
      if (request.method === "GET" && url.pathname === "/api/ai/policy-builder-status") {
        return json(await getAiBuilderStatus(db));
      }
...
let sharedDb: VotingDatabase | null = null;

export const getSharedDatabase = () => {
  if (!sharedDb) {
    sharedDb = openVotingDatabase();
  }
